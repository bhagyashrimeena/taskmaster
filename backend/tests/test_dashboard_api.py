...
from wealth_copilot.dashboard.service import DashboardService, attention_message_for_count
...
async def test_dashboard_bootstrap_is_immediate_and_complete() -> None:
    ...
    assert elapsed < 1.0
    assert dashboard.portfolio.source.label == "Demo portfolio"
    assert dashboard.day_id.endswith(application_today().isoformat())
    assert dashboard.portfolio.portfolio_value > 0
    assert len(dashboard.daily_brief.stories) == 5
    assert dashboard.daily_brief.freshness.status == FreshnessStatus.CACHED
    assert dashboard.important_event is not None
    assert dashboard.important_event.decision.value == "ALERT"
    assert dashboard.important_event.affected_portfolio_percentage > 0
    assert dashboard.important_event.relevance_score >= 80
...
def test_frontend_contains_no_investment_recommendation_language() -> None:
    ...
    banned = re.compile(
        r"\b(?:buy|sell|rebalance)\b|\b(?:should|recommend(?:ed|s|ing)?)\s+hold\b|\bhold\s+(?:this|the)\s+(?:stock|security|position)\b",
        re.IGNORECASE,
    )
    assert banned.search(source) is None
    assert "Score calculated by rules. Explanation generated by AI. You decide." in source


def test_attention_message_uses_correct_singular_and_plural_grammar() -> None:
    assert attention_message_for_count(1) == "1 thing deserves your attention today"
    assert attention_message_for_count(2) == "2 things deserve your attention today"
